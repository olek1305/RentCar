<x-layout>
    <x-slot:title>{{ __('messages.car_edit') }}</x-slot:title>

    <section class="container mx-auto p-6 max-w-3xl">
        <h1 class="text-3xl font-bold mb-8 text-center text-gray-800">
            {{ __('messages.car_edit') }}
        </h1>
        ...
        <form method="POST" action="{{ route('cars.update', $car->id) }}" enctype="multipart/form-data" class="bg-white p-6 rounded-xl shadow-md space-y-6">
            ...
            <div>
                ...
                <input type="number" name="daily_price" value="{{ old('daily_price', $car->rental_prices['1-2'] ?? '') }}"
            ...
            <div>
                ...
                @if($car->main_image)
                    <div class="mb-2 relative">
                        ...
                        <!-- Add this checkbox for deleting old main image -->
                        <div class="absolute top-2 right-2 bg-white p-1 rounded shadow">
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="delete_old_main_image" value="1" class="rounded">
                                <span class="text-xs">{{ __('messages.delete_image') }}</span>
                            </label>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="grid grid-cols-3 gap-3 mb-2">
                        @foreach($galleryImages as $index => $image)
                            ...
                            <div class="relative group">
                                <img src="{{ Storage::url($image) }}" alt="Gallery image {{ $index }}"
